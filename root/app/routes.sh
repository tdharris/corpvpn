...
log debug "docker_interface:${docker_interface} docker_ip:${docker_ip} docker_gw:${docker_gw}"

vpn_ip="$(ifconfig tun | grep -P -o -m 1 '(?<=inet\s)[^\s]+')"
vpn_gw="$(ip route show ${vpn_ip} | awk '{print $1}')"
log debug "vpn_ip:${vpn_ip} vpn_gw:${vpn_gw}"

log info "Setting up default route through ${vpn_gw}"
...
for lan_item in "${lan_array[@]}"; do
    ...
    log info "Adding LAN_NETWORK ${lan_item} as route via docker ${docker_interface}"
